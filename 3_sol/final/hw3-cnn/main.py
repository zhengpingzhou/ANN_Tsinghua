...
def train_epoch(model, sess, X, y, summary_writer=None):
    loss, acc = 0.0, 0.0
...
def valid_epoch(model, sess, X, y, summary_writer=None):
    loss, acc = 0.0, 0.0
    st, ed, times = 0, FLAGS.batch_size, 0
    while st < len(X) and ed <= len(X):
        X_batch, y_batch = X[st:ed], y[st:ed]
...
with tf.Session() as sess:
    ...
    if FLAGS.is_train:
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            train_acc, train_loss = train_epoch(cnn_model, sess, X_train, y_train, summary_writer_train)
            X_train, y_train = shuffle(X_train, y_train, 1)

            val_acc, val_loss = valid_epoch(cnn_model, sess, X_val, y_val, summary_writer_valid)
